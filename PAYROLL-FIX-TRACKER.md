...
- [ ] **S12** Fix compliance flag assignment — don't assume 8 checks per item (`compliance.service.ts:342`)
- [ ] **S13** Wrap loan repayment creation in transaction (`loans.service.ts:105`)
- [ ] **S14** Fix `req.user!._id` → `req.user!.id` in payroll-reports (`payroll-reports.controller.ts:16`)
