...
export class UsersService {
  ...
  static async create(data: Record<string, unknown>, createdById: string) {
    ...
    const wasAutoGenerated = !data.password;
  ...
  static async importUsers(users: Array<{ name: string; email: string; password?: string; role: string }>, createdById: string) {
    const results = { created: 0, updated: 0, errors: [] as string[], credentials: [] as Array<{ email: string; password: string }> };
    ...
    for (const userData of users) {
      try {
        ...
        if (existing) {
          ...
          // Auto-generate password if not provided (for workers/bulk imports)
          const password = userData.password || generatePassword();
          const wasAutoGenerated = !userData.password;
          ...
          // Track auto-generated credentials for workers
          if (wasAutoGenerated) {
